<template>
  <MainLayout>
    <header class="mv-header mv-header-bg px-3 px-sm-4">
      <h1 class="fw-bold text-primary">About Multividas</h1>
      <h4 class="fw-bold text-muted my-2">
        Share your Threads, Write and publish your Articles
      </h4>
    </header>
    <main class="container px-4 py-2 my-4 py-md-5 my-md-2">
      <div class="card mb-3">
        <div class="card-header">
          <h3 class="text-primary fw-bold">Who we are?</h3>
        </div>
        <div class="card-body">
          <p>
            <a href="https://multividas.com" target="_blank" class="fw-bold text-primary text-decoration-underline">Multividas.com</a> began in ceo-founder
            <a
              href="https://soulaimaneyahya.com/"
              target="_blank"
              class="fw-bold text-primary text-decoration-underline"
              >Soulaimane Yahya</a
            >
            living room and neighborhood library in 2023.
          </p>
        </div>
      </div>

      <div class="card mb-3">
        <div class="card-header">
          <h3 class="text-primary fw-bold">We are on a mission!</h3>
        </div>
        <div class="card-body">
          <p>
            Our mission is to build the app of everything, creating a diverse
            and inclusive world for everyone to share short and long blog posts.
          </p>
        </div>
      </div>

      <div class="card mb-3">
        <div class="card-header">
          <h3 class="text-primary fw-bold">Share Your Threads</h3>
        </div>
        <div class="card-body">
          <p>
            With <a href="https://multividas.com" target="_blank" class="fw-bold text-primary text-decoration-underline">Multividas.com</a>, you can share short texts and
            posts. Our platform encourages engagement through comments and
            replies within threaded posts.
          </p>
        </div>
      </div>

      <div class="card mb-3">
        <div class="card-header">
          <h3 class="text-primary fw-bold">Write and publish your Articles</h3>
        </div>
        <div class="card-body">
          <p>
            With <a href="https://multividas.com" target="_blank" class="fw-bold text-primary text-decoration-underline">Multividas.com</a>, you can write and publish long articles. Our platform encourages engagement through comments and
            replies within long articles.
          </p>
        </div>
      </div>

      <div class="card mb-3">
        <div class="card-header">
          <h3 class="text-primary fw-bold">Hashtags and Trending Topics</h3>
        </div>
        <div class="card-body">
          <p>
            Discover, engage with, and stay informed about the latest trending
            threads and posts by exploring hashtags and topics. You can also
            search for specific topics of interest to you.
          </p>
        </div>
      </div>

      <div class="card mb-3">
        <div class="card-header">
          <h3 class="text-primary fw-bold">Anonymous Threads</h3>
        </div>
        <div class="card-body">
          <p>
            You can share anonymous threads without displaying your name, profile image, or personal identities.
          </p>
        </div>
      </div>

      <div class="card mb-3">
        <div class="card-header">
          <h3 class="text-primary fw-bold">Anonymous Comments</h3>
        </div>
        <div class="card-body">
          <p>
            You can leave comments and having discussions on threads anonymously.
          </p>
        </div>
      </div>

      <div class="card">
        <div class="card-header">
          <h3 class="text-primary fw-bold">Privacy and Secure Communication</h3>
        </div>
        <div class="card-body">
          <ul class="multividas-list mx-3">
            <li class="px-2 mb-3">All your personal informations, username, phone number, email, ... are encrypted</li>
            <li class="px-2 mb-3">All your private threads are encrypted</li>
            <li class="px-2 mb-3">All your anonymous threads are encrypted</li>
            <li class="px-2 mb-3">All you anonymous comments are encrypted</li>
            <li class="px-2 mb-3">Your email and phone number remain private, inaccessible to anyone, unless you choose to make them public in your settings.</li>
            <li class="px-2">Ensuring privacy and security are our top priorities at <a href="https://multividas.com" target="_blank" class="fw-bold text-primary text-decoration-underline">Multividas.com</a></li>
          </ul>
        </div>
      </div>
    </main>
  </MainLayout>
</template>

<script>
export default {
  metaInfo: {
    title: "About Page",
  },
};
</script>

<style scoped></style>
